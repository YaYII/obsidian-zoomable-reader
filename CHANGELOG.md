...
- **图里的文字固定 ≤16px，不再被「拉伸」放大。** 用户原话：「你的插件和主题在生成 mermaid 里面的
  文字打小应该是固定的，不能超过 16px 的大小」。根因是 1.3.9 那条「图表撑满版心」的规则：
  它把 svg 连字带线一起放大 —— 一张 478px 的图铺进 900px 版心是 1.9 倍（白板 1280px 时 2.7 倍），
  图里 16px 的字渲染成 30~43px。现在图表只保证「不溢出」（max-width）：窄图保持原尺寸，
  宽图按比例缩进版心；主题侧把图内字号改成固定令牌（节点 14px、边标签与时序消息 12.5px），
  不再跟着「设置 → 外观 → 字体大小」和分辨率增益浮动。
- **长标签自动折行，不再「一行顶一个宽度」。** 用户原话：「图形里面的文字内容太多了，你应该可以
  换行呀，而不是一行顶一个宽度呀」。查证（Mermaid 10.9 实测）：**普通标签 A[长文本] 从不折行** ——
  把 flowchart.wrappingWidth 从 460 改成 120，渲染结果一模一样（同一个 478×42 的框、同一行字）。
  也就是说，过去插件里那个「标签最大宽度」滑块对普通标签其实是个空旋钮，这正是抱怨的根源。现在：
  - 插件把**长标签改写成 markdown 字符串**（A["`长文本`"]）—— 只有这种标签 Mermaid 才按折行宽度排版；
    同时打开 markdownAutoWrap，默认折行宽度 460 → **260**（一行约 16 个汉字，长标签折成 2~3 行）；
  - 白板 / 版面视图在**渲染前**改写（wrapMermaidFences）；**阅读视图**由 Markdown 后处理器接管：
    用 context.getSectionInfo 取回该块源码 → 改写 → 自己重渲染换上去。任何一步失败都保持宿主
    原本那张图不换 —— 这块代码最坏的结局必须是「没折行」，而不是「图没了」；
  - 真实渲染实测（真 Mermaid + 真主题 CSS）：同一个 43 字标签，改写前 **1 行、框 678px**；
    改写后 **3 行、框 278px**，字号不变（14.96px），文字不溢出框。

Verification: 单元测试 **100** 条（新增 tests/mermaid-labels.test.ts 十条：只改长标签、短标签一字不动、
markdown 字符串不重复包装、边标签、幂等、围栏提取）；npm run verify:layout 新增 6 条**真实渲染**断言
（改写前后的行数 / 框宽 / 字号 / 溢出）；npm run verify:reading **37** 条（「流程图按版心宽度显示」
那条按用户的新要求翻过来，改成断言保持自然宽度 478px）。

## 1.3.11

- **查看器里给的是「原图」，不是正文里那张缩略图。** 用户原话：「查看图片的时候，放大后，点击图片的时候，
  你需要确定放大的图片是原图高清的」。正文里的 `<img>` 可能带 `srcset`（浏览器按正文的显示尺寸 × DPR
  挑一张），也可能被懒加载插件塞了 1px 占位图、真图在 `data-src` 里。查看器现在自己挑：
`srcset` 里取分辨率最高的一张，src 是占位图就用 `data-src`。顺带修掉一个真 bug ——
  **`srcset` 不能按逗号 split**：`data:` URL 里全是逗号，切完每个候选都成了半截地址
  （在验证台里的表现是「查看器的图自然尺寸 0」）。
- **右上角多了一个关闭按钮。** 用户要求：「放大后的图片，右上角应该有一个关闭的按钮，方便关闭放大的图」。
  它钉在图片区域的右上角（跟着安全区走），与图片的缩放平移无关 —— 图拖到哪儿它都在同一个位置；
  桌面 34px、手机 44px（Apple HIG 的最小可点尺寸），鼠标 / 触摸 / `Esc` 三条路都能关。
  工具条里那个 12px 的小叉已去掉：两个关闭按钮只会让人犹豫。
- **点一下图片 = 落到原图 100%**（已经在原图了就收回「适配」）。这是「点击图片 → 确认是原图高清」最直接的
  落点：点完读数会写明 `1600 x 900 px → 1600 x 900 px · 原图 / native`。
  三条防误触：位移 ≤ 8px、时长 ≤ 500ms、全程单指；拖动平移松手那一下不会触发
  （手势层一旦判定为拖动就会 setPointerCapture，pointerup 根本回不到内容元素上）。
  双击仍然是放大到 200%，两条路不打架。

Verification: 单元测试 **88** 条（新增 `parseSrcset` / `pickHiResSource`：data URL 里的逗号、
1x/2x 描述符、懒加载占位图、相对地址拼接）；`npm run verify:gestures` **93** 条断言，
其中 8 条新断言先在旧代码上跑出红色：懒加载只拿到 1x1、关闭按钮不存在（桌面与手机各两条）、
点图片没反应（0.533 → 0.533）、手机触摸图片同样没反应（0.642），红的时候 85/93。
诚实边界：无头 Chromium 自己给 srcset 挑的就是最大的那张，所以浏览器侧那条「拿到 1600px 原图」
证明的是最终结果；「比浏览器更会挑」这条逻辑由 `tests/lightbox.test.ts` 的单测钉住。

## 1.3.10

- **手机上终于看得见那个放大的 icon 了 —— 这一条是用户直接点名的。** 触屏设备上按钮以前是
  `@media (hover: none) { display: none }` 被整个藏起来的，理由是「轻点图片就能放大，按钮不必占地方」。
  用户的回答：「你应该展示放大的 icon，这样我就可以点击了，而不是依靠点击图片，因为图片点击的时候
  可能点击到了背景上去了，所以体验不是很好」。现在触屏上按钮**常驻可见**（不再靠悬停提亮：触屏没有
  悬停，0.42 的不透明度等于看不见），视觉 34px，触区用 `::after` 外扩到约 46px —— 手指按不准也点得到。
  轻点图片这条入口保留，但按钮才是明确的那一个。
- **连点两下按钮不再进编辑。** 按钮就贴在图片角上，而「双击」的第二次 tap 正是 Obsidian
  「双击进入编辑」的触发点：现在第二次 tap 会被吃掉（第一次照旧打开查看器，且只打开一次）。
- **查看器里的图现在是高清的，不是被压缩后再拉大的。** 两个独立原因，都修了：
  - **100% 的基准错了**：以前「1 个源像素 = 1 个 CSS 像素」，所以在 DPR=3 的手机上，一张 1600px 的
    截图被铺到 4800 个物理像素上 —— 横向拉伸 3 倍，这就是用户说的「放大查看都模糊了」。现在按
    `naturalWidth / devicePixelRatio` 排版：**100% 就是原图分辨率**（1 源像素 = 1 设备像素），读数也
    对得上（`1600 x 900 px → 1600 x 900 px`）。再往上放大才是放大镜。
  - 画布上带着 **`will-change: transform`**：它把画面钉死在提层时的比例上，之后放大只是把旧位图拉大
    （照片发软、图表里的 CSS 文字直接糊）。已去掉。
  - 另外，**打开小图不再「适配」放大**（适配比例封顶 100%），并解除主题/宿主塞给 img 的
    `max-width: 100%`（它会把刚摆好的 1:1 又压回去，窄屏上高清立刻失效）。
- 细节：插件的手机手势拦截器现在会避开自己的放大按钮 —— 按钮里的图标是 `<svg>`，不排除就会被
  「这是不是图表」的判定当成一张 18px 的图表打开。

Verification: 单元测试 **82** 条（新增 `tests/lightbox.test.ts` 钉住 1:1 的换算，含 devicePixelRatio
不老实的情况）；`npm run verify:gestures` **82** 条断言，`npm run verify:reading` **36** 条断言。
新断言先在旧代码上跑过一遍确认它们能失败：1.3.9 上触屏的按钮是 `display=none opacity=0.42 0x0`、
小图打开是 426%、`will-change` 计算值是 `transform`。
