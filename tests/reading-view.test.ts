...
describe("readingCss：注入的样式", () => {
  it("默认（900px + 100%）：改版心宽度变量，并给 sizer 兜底", () => {
    const css = readingCss({ lineWidth: 900, zoom: 1, gestures: true });
  ...
  it("缩放时用 CSS zoom（会重排），并把 sizer 宽度按比例除掉 —— 字变大、行宽不变", () => {
    const css = readingCss({ lineWidth: 900, zoom: 1.5, gestures: true });
  ...
  it("「跟随主题」：不注入任何宽度规则，只可能注入 zoom", () => {
    const css = readingCss({ lineWidth: "theme", zoom: 1.25, gestures: true });
  ...
  it("跟随主题 + 100% + 手势开：只声明 touch-action（否则双指会被浏览器抢走）", () => {
    const css = readingCss({ lineWidth: "theme", zoom: 1, gestures: true });
  ...
  it("手势关掉：不声明 touch-action（一个字节都不注入）", () => {
    expect(readingCss({ lineWidth: "theme", zoom: 1, gestures: false })).toBe("");
  });

  it("选择器限定在阅读视图内（本插件自己的视图不受影响）", () => {
    const css = readingCss({ lineWidth: 900, zoom: 1.2, gestures: true });
